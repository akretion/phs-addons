...
class Dispute(models.Model):
    ...
    def _compute_model_ref_id_model_name(self):
        self.model_ref_id_model_name = (
            self.model_ref_id and self.model_ref_id._name or False
        )

    @api.depends("model_ref_id")
    def _compute_model_ref_id_selected_id(self):
        self.model_ref_id_selected_id = (
            self.model_ref_id and self.model_ref_id.id or False
        )
    ...
    def get_line_model_info(self, model_name):
        """return a list of dictionary with all the informations to handle the lines
        reference field

        :param
            char model_name: model_name of the dispute to limit the return to only
            compatible the lines model linked

        :return list of dict return: {'model': model_name, 'domain': odoo domain to
            apply on the field reference, 'product_id': name of the field product_id
    ...
    dispute_type = fields.Char(
        compute="_compute_type", store=True, states=READONLY_STATES
    )
    ...
    model_ref_id_selected_id = fields.Integer(
    ...
    def action_dispute_send(self):
        """
        This function opens a window to compose an email, with the dispute
        ...
        try:
            template_id = ir_model_data.get_object_reference(
                "dispute", "email_template_dispute"
            )[1]
        except ValueError:
            template_id = False
        try:
        ...
        # In the case of a dispute, we want the "View..." button in line with
        #  the state of the object. Therefore, we pass the model description in
        # the context, in the language in which the template is rendered.
    ...
    def message_post(self, **kwargs):
        if self.env.context.get("mark_dispute_as_sent"):
            self.filtered(lambda o: o.state in ["draft", "in_progress"]).write(
                {"state": "sent"}
            )
        return super(
            Dispute, self.with_context(mail_post_autofollow=True)
        ).message_post(**kwargs)
...
class DisputeLine(models.Model):
    ...
    def _selection_model(self):
        ...
        for info in self.env["dispute"].get_line_model_info(None):
            ...
            selection.append(
                (line_model_name, _(self.env[line_model_name]._description))
            )
    ...
    def _compute_product_id(self):
        for r in self:
            ...
            if r.dispute_id and r.model_ref_id:
                ...
                product_field = next(
                    (
                        info["product_id_field_name"]
                        for info in line_model_info
                        if info["model_name"] == r.model_ref_id._name
                    ),
                    None,
                )
                if product_field:
    ...
    def fields_get(self, allfields=None, attributes=None):
        ...
        context_model_ref_id_model_name = self._context.get(
            "model_ref_id_model_name", False
        )
        context_model_ref_id_selected_id = self._context.get(
            "model_ref_id_selected_id", []
        )

        model_name = context_model_ref_id_model_name
        dispute_model_ref_id = (
            model_name
            and context_model_ref_id_selected_id
            and self.env[model_name].browse(context_model_ref_id_selected_id)
            or False
        )

        # fields = self.fields_get(allfields, attributes)
        fields = super(DisputeLine, self).fields_get(allfields, attributes=attributes)
        ...
        fields["model_ref_id"]["selection"] = [
            (
                info.get("model_name", ()),
                _(self.env[info.get("model_name", ())]._description),
            )
            for info in line_model_info
        ]
        if dispute_model_ref_id:
            ...
            for info in line_model_info:
                if len(domain):
                    domain.append("|")
                for dom in safe_eval(info.get("domain", []),locals_dict={'dispute_model_ref_id': dispute_model_ref_id}):
                    domain.append(dom)
...
class DisputeLineMixin(models.AbstractModel):
    ...
    dispute_comment = fields.Text(related="dispute_line_id.comment", readonly=False)
